...
public class RequestInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        boolean logResponse_code = (printLevel != Level.NONE && printLevel == Level.RESPONSE_CODE);
        ...
        if (logResponse||logResponse_code) {
            //打印响应结果
    ...
    private String parseContent(ResponseBody responseBody, String encoding, Buffer clone) {
        ...
        if (encoding != null && encoding.equalsIgnoreCase("gzip")) {//content 使用 gzip 压缩
            return HttpZipHelper.decompressForGzip(clone.readByteArray(), convertCharset(charset));//解压
        } else if (encoding != null && encoding.equalsIgnoreCase("zlib")) {//content 使用 zlib 压缩
            return HttpZipHelper.decompressToStringForZlib(clone.readByteArray(), convertCharset(charset));//解压
