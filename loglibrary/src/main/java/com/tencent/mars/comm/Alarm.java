...
public class Alarm extends BroadcastReceiver {
    ...
    public static boolean start(final long id, final int after, final Context context) {
        ...
        if (0 > after) {
            XLogWrapper.e(TAG, "id:%d, after:%d", id, after);
            return false;
        }

        if (null == context) {
            XLogWrapper.e(TAG, "null==context, id:%d, after:%d", id, after);
            return false;
        }
        ...
        synchronized (alarm_waiting_set) {
            if (null == wakerlock) {
                ...
                XLogWrapper.i(TAG, "start new wakerlock");
            }
            ...
            while (it.hasNext()) {
                if ((Long) (it.next()[TSetData.ID.ordinal()]) == id) {
                    XLogWrapper.e(TAG, "id exist=%d", id);
                    return false;
                }
            }
    ...
    public static boolean stop(final long id, final Context context) {
        ...
        if (null == context) {
            XLogWrapper.e(TAG, "context==null");
            return false;
        }
        ...
        synchronized (alarm_waiting_set) {
            if (null == wakerlock) {
                wakerlock = new WakerLock(context);
                XLogWrapper.i(TAG, "stop new wakerlock");
            }
            ...
            if (null == bc_alarm) {
                ...
                XLogWrapper.i(TAG, "stop new Alarm");
            }
    ...
    private static boolean cancelAlarmMgr(final Context context, PendingIntent pendingIntent) {
        ...
        if (pendingIntent == null) {
            XLogWrapper.e(TAG, "pendingIntent == null");
            return false;
        }
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        if (pid != Process.myPid()) {
            XLogWrapper.w(TAG, "onReceive id:%d, pid:%d, mypid:%d", id, pid, Process.myPid());
            return;
        }
        ...
        synchronized (alarm_waiting_set) {
            ...
            while (it.hasNext()) {
                ...
                XLogWrapper.i(TAG, "onReceive id=%d, curId=%d", id, curId);
                if (curId.equals(id)) {

                    XLogWrapper.i(TAG, "onReceive find alarm id:%d, pid:%d, delta miss time:%d", id, pid, SystemClock.elapsedRealtime() - (Long) next[TSetData.WAITTIME.ordinal()]);
                    it.remove();
                    found = true;
                    break;
                }
            ...
            if (!found)
                XLogWrapper.e(TAG, "onReceive not found id:%d, pid:%d, alarm_waiting_set.size:%d", id, pid, alarm_waiting_set.size());
        }
